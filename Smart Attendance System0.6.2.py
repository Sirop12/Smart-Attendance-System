...
def format_mode():
    ...
    print("Correct password:", "d1e2a39c0d7939eb0f98c97412712e583e6922b9be3bda24ae0c6523d0226a3c")
    if hashlib.sha256(
            password.encode()).hexdigest() == "d1e2a39c0d7939eb0f98c97412712e583e6922b9be3bda24ae0c6523d0226a3c":
        format_ = ''
...
def admin_mode():
    ...
    if hashlib.sha256(password.encode()).hexdigest() == getData("password"):
        ...
        search_button = ttk.Button(search_frame, text="Search", command=lambda: search_user(search_var, tree))
        search_button.pack(side=tk.LEFT, padx=5, pady=5)

        clear_search_button = ttk.Button(search_frame, text="Clear Search", command=lambda: clear_search(search_var, tree))
        clear_search_button.pack(side=tk.LEFT, padx=5, pady=5)

        tree = ttk.Treeview(users_frame, columns=('ID', 'FstName', 'LstName', 'poto_path'), show='headings')
        tree.heading('ID', text='ID')
        tree.heading('FstName', text='FstName')
        tree.heading('LstName', text='LstName')
        tree.heading('poto_path', text='poto_path')
        ...
        data_tree = ttk.Treeview(data_frame, columns=('Key', 'Value'), show='headings')
        ...
        def update_data():
            ...
            key = data_tree.item(selected_item)['values'][0]
            value = prompt_input("Update Data", "Enter new value for key '{}':".format(key))
            if value:
                cursor.execute("UPDATE Data SET Value = ? WHERE Name = ?", (value, key))
                connect.commit()
                update_data_list(data_tree)
